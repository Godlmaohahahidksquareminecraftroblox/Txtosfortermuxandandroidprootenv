...
# Prepare directories
mkdir -p "$TXTOS_DIR/lib" "$TXTOS_DIR/system" "$TXTOS_DIR/bin"

# Gather basic linker & shell
ldd /bin/sh | awk '{print $3}' | xargs -I {} cp -u {} "$TXTOS_DIR/lib" || true
cp -u /bin/sh "$TXTOS_DIR/bin"
cp -u /bin/linker64 "$TXTOS_DIR/bin"
cp -u /bin/linker64 "$TXTOS_DIR/lib"

# Copy into system
cp -r "$TXTOS_DIR/bin" "$TXTOS_DIR/system"
cp -r "$TXTOS_DIR/lib" "$TXTOS_DIR/system"

# Gather required libraries
cd "$TXTOS_DIR/lib"
cp /apex/com.android.runtime/lib64/bionic/libc.so ./ || true
cp /system/lib64/libm.so ./ || true
cp /system/lib64/libdl.so ./ || true
cp /system/lib64/liblog.so ./ || true
cp /system/lib64/libstdc++.so ./ || true
cp /system/lib64/libz.so ./ || true
cp /system/lib64/libandroid.so ./ || true
cp /system/lib64/libbinder.so ./ || true
cp /system/lib64/libutils.so ./ || true
cp /system/lib64/libcutils.so ./ || true
cp /system/lib64/libcrypto.so ./ || true
cp /system/lib64/libssl.so ./ || true
cp /system/lib64/libsqlite.so ./ || true
cp /system/lib64/libc++.so ./ || true
cp /system/lib64/libbase.so ./ || true
cp /system/lib64/libcgrouprc.so ./ || true
cp /system/lib64/libpcre2.so ./ || true
cp /system/lib64/libpackagelistparser.so ./ || true
cp /system/lib64/libprocessgroup.so ./ || true
cp /system/lib64/libselinux.so ./ || true
...
grep -oE '"/apex[^"]+"' log.txt | tr -d '"' | sort -u | while read path; do
    if [ -e "$path" ]; then
        cp -ru --parents "$path" "$TXTOS_DIR" 2>/dev/null || true
    fi
done

# Set up toybox
TOYBOX_PATH="$TXTOS_DIR/bin/toybox"
cp /system/bin/toybox "$TOYBOX_PATH"
chmod +x "$TOYBOX_PATH"
